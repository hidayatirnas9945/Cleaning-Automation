...
# projectid = 409
#
# # reading question logic from database
# # Question = pd.read_csv('Sorted Question&Logic.csv', low_memory=False)
# query = '''select pt.ProjectId PROJECT_ID, pt.code PROJECT_CODE,
# st.Counter SECTION_COUNTER, qt.Counter QUESTION_COUNTER, -- pt.name PROJECT_NAME,
# st.code SECTION_CODE, -- st.name SECTION_NAME,
# qt.code QUESTION_CODE, -- qt.name QUESTION_NAME,
# qit.code ANSWER_CODE, -- qit.name ANSWER_NAME,
# prt_qtype.Code QUESTION_TYPE, prt_logic.Code QUESTION_LOGIC, st_next.Code NEXT_SECTION_CODE,
# qt_next.Code NEXT_QUESTION, prt_next_qtype.Code NEXT_QUESTION_TYPE
# from
# questiontbl qt join sectiontbl st on qt.SectionId = st.SectionId and qt.QuestionStatusId = 2947
# join projecttbl pt on pt.ProjectId = st.ProjectId and st.ProjectId = %d
# join questionitemtbl qit on qt.QuestionId = qit.QuestionId
# left join questionlogictbl qlt on qlt.QuestionItemId = qit.QuestionItemId
# left join parametertbl prt_logic on prt_logic.ParameterId = qlt.LogicTypeId
# left join questiontbl qt_next on qlt.NextQuestionId = qt_next.QuestionId
# left join parametertbl prt_qtype on qt.QuestionTypeId = prt_qtype.ParameterId
...
# order by SECTION_COUNTER, QUESTION_COUNTER;''' % (projectid)
#
# engine = create_engine('mysql+pymysql://hidayat9939:6$=J9dHIIWk}0y=@103.197.57.78:3306/tess_v60')
# Question = pd.read_sql(query, engine)
# Question = Question.applymap(lambda x: gc.convertToIntOrString(x)).sort_values(by=['SECTION_COUNTER', 'QUESTION_COUNTER'])
# Question.fillna('NONE', inplace = True)
# QuestionGroup = Question.groupby(['SECTION_COUNTER', 'QUESTION_COUNTER', 'SECTION_CODE', 'QUESTION_TYPE', 'QUESTION_CODE', 'QUESTION_LOGIC', 'NEXT_SECTION_CODE', 'NEXT_QUESTION', 'NEXT_QUESTION_TYPE'])
# # QuestionGroup_CheckNextQuestion = Question[['QUESTION_CODE', 'QUESTION_LOGIC', 'ANSWER_CODE']][~Question['QUESTION_LOGIC'].isin(['TIPA', 'JQPA', 'SQPA'])].groupby('QUESTION_CODE')
# tmpQuestionList = [tuple(x) for x in Question[['SECTION_CODE', 'QUESTION_CODE', 'QUESTION_TYPE']].values]
# QuestionList = []
# for eachTuple in tmpQuestionList:
#     if not eachTuple in QuestionList:
#         QuestionList.append(eachTuple)
# del tmpQuestionList


# # reading data
# df = pd.read_csv('pr409.csv', low_memory=False)
# df, dfColumns = gc.cleaningColumns(df)
# df = gc.cleaningRows(df)
...
#         if not questionCode in QuestionDoneChecked:
#             if questionType == 'RADIOBUTTON':
#                 answerList = Question[['QUESTION_CODE', 'ANSWER_CODE']].groupby('QUESTION_CODE').get_group(questionCode)['ANSWER_CODE'].unique().tolist() + [np.nan]
#                 df = gc.checkRejectStatus_RadioButton(df, projectid, questionCode)
#                 df = gc.checkValueRange(df, projectid, questionCode, answerList)
#                 df = gc.checkSpecify_RadioButton(df, projectid, questionCode)
#             elif questionType == 'CHECKBOX':
...
#
#         if questionLogic == 'TIPA':
#             answerList = QuestionGroup.get_group((sectionCounter, questionCounter, sectionCode, questionType, questionCode, questionLogic, nextSectionCode, nextQuestion, nextQuestionType))['ANSWER_CODE'].unique().tolist()
#             df = gc.TIPA(df, projectid, questionCode, answerList)
#         elif questionLogic == 'JQPA':
#             startIndex = QuestionList.index((sectionCode, questionCode, questionType)) + 1
#             endIndex = QuestionList.index((nextSectionCode, nextQuestion, nextQuestionType))
...
#             answerList = QuestionGroup.get_group((sectionCounter, questionCounter, sectionCode, questionType, questionCode, questionLogic, nextSectionCode, nextQuestion, nextQuestionType))['ANSWER_CODE'].unique().tolist()
#             if questionType == 'RADIOBUTTON':
#                 df = gc.JQPA_RadioButton(df, projectid, questionCode, answerList, QuestionListToPassOver, QuestionCodeDict)
#             elif questionType == 'CHECKBOX':
#                 df = gc.JQPA_CheckBox(df, projectid, questionCode, answerList, QuestionListToPassOver, QuestionCodeDict)
#         elif nextQuestion in QuestionCodeDict.keys():
#             if questionLogic == 'SAPA':
#                 df = gc.SAPA(df, projectid, questionCode, nextQuestion, nextQuestionType, QuestionCodeDict)
#             elif questionLogic == 'HAPA':
#                 df = gc.HAPA(df, projectid, questionCode, nextQuestion, nextQuestionType, QuestionCodeDict)
#             elif questionLogic == 'SQPA':
#                 answerList = QuestionGroup.get_group((sectionCounter, questionCounter, sectionCode, questionType, questionCode, questionLogic, nextSectionCode, nextQuestion, nextQuestionType))['ANSWER_CODE'].unique().tolist()
#                 if questionType == 'RADIOBUTTON':
#                     df = gc.SQPA_RadioButton(df, projectid, questionCode, answerList, nextQuestion, nextQuestionType, QuestionCodeDict)
#                 elif questionType == 'CHECKBOX':
#                     df = gc.SQPA_CheckBox(df, projectid, questionCode, answerList, nextQuestion, nextQuestionType, QuestionCodeDict)
#             elif questionLogic == 'HQPA':
#                 answerList = QuestionGroup.get_group((sectionCounter, questionCounter, sectionCode, questionType, questionCode, questionLogic, nextSectionCode, nextQuestion, nextQuestionType))['ANSWER_CODE'].unique().tolist()
#                 if questionType == 'RADIOBUTTON':
#                     df = gc.HQPA_RadioButton(df, projectid, questionCode, answerList, nextQuestion, nextQuestionType, QuestionCodeDict)
#                 elif questionType == 'CHECKBOX':
#                     df = gc.HQPA_CheckBox(df, projectid, questionCode, answerList, nextQuestion, nextQuestionType, QuestionCodeDict)